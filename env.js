...
const config = envok({
  ...
  BB_USER_API_ENDPOINT: Joi.string().uri({scheme: ['http']}).required(),
  BB_CAMPAIGN_API_ENDPOINT: Joi.string().uri({scheme: ['http']}).required(),
  BB_DONATIONS_API_ENDPOINT: Joi.string().uri({scheme: ['http']}).required(),
